...
class BoundaryGateTest(unittest.TestCase):
    ...
    def test_redacted_report_value_mode_hides_raw_values_in_outputs(self) -> None:
        trace = load_trace(ROOT / "samples/traces/claim_agent_minimal.jsonl")
        policy = load_policy(ROOT / "samples/policies/default.yml")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["report_value_mode"], "redacted")
            self.assertNotIn("MBR-SYN-8842", markdown)
            self.assertNotIn("MBR-SYN-8842", serialized)
            self.assertIn("[REDACTED_MEMBER_ID]", markdown)
            self.assertEqual(payload["findings"][0]["value"], payload["findings"][0]["value_display"])
            self.assertTrue(payload["findings"][0]["value_hash"].startswith("sha256:"))

    def test_hashed_report_value_mode_uses_stable_hashes(self) -> None:
        trace = load_trace(ROOT / "samples/traces/claim_agent_minimal.jsonl")
        policy = load_policy(ROOT / "samples/policies/default.yml")
        ...
        member_findings = [finding for finding in report["findings"] if finding["category"] == "member_id"]
        member_hashes = {finding["value"] for finding in member_findings}

        self.assertEqual(report["report_value_mode"], "hashed")
        self.assertEqual(len(member_hashes), 1)
        self.assertTrue(next(iter(member_hashes)).startswith("sha256:"))
        self.assertNotIn("MBR-SYN-8842", json.dumps(report))

    def test_cli_report_values_redacted_hides_raw_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            markdown_path = tmp_path / "report.md"
            json_path = tmp_path / "report.json"
            ...
            self.assertEqual(exit_code, 0)
            self.assertNotIn("MBR-SYN-8842", markdown_path.read_text(encoding="utf-8"))
            self.assertNotIn("MBR-SYN-8842", json_path.read_text(encoding="utf-8"))
